...
public class Failed extends Status {
    ...
    public void setAlternativeTask(Task task, Task alternativeTask) throws IllegalStateException, IllegalArgumentException {
        if (task.getAlternativeTask() != null) {
            throw new IllegalStateException("Can't set an alternative task for this task because "
                    + "this task already has an alternative task.");
        }
        if (alternativeTask == task) {
            throw new IllegalArgumentException("Can't set an alternative task for this task because "
        ...
        if (!task.getProject().hasTask(alternativeTask)) {
            throw new IllegalArgumentException("This task can't have the given task as its alternative task.");
        }
    ...
    /**
     * @return {@code false} if this task doesn't have an alternative task.
     *         {@code true} if this task has an alternative task that can be fulfilled.
     */
    @Override
